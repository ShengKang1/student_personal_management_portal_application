...
//Drop down meneu list
  Map<String, List<String>> programmeSpecializations = {};
  late List<String> programme_dropdownItems;
  late List<String> specialization_dropdownItems = [];
  late String selectedProgramme = "";
  late List<String> intake_dropdownItems = [];

  @override
  void initState() {
    super.initState();
    // programme_dropdownItems = [];
    fetchProgrammeDropdownItems();
    fetchIntakeData();
  }

  Future<void> fetchProgrammeDropdownItems() async {
    try {
      ...
      programmeSpecializations =
          data.map((key, value) => MapEntry(key, List<String>.from(value)));

      setState(() {
        programme_dropdownItems = programmeSpecializations.keys.toList();
        if (selectedProgramme.isNotEmpty) {
          specialization_dropdownItems =
              programmeSpecializations[selectedProgramme]!;
        } else {
          specialization_dropdownItems = [];
        }
      });
    } catch (e) {
      print('Error fetching programme specializations: $e');
    }
  }

  Future<List<String>> fetchSpecializationData(String programme) async {
    try {
      DocumentSnapshot documentSnapshot = await FirebaseFirestore.instance
          .collection('dropdown_data')
          .doc('programme')
      ...
      List<String> specializations = data[programme].cast<String>();

      return specializations;
    } catch (e) {
      print('Error fetching specialization data: $e');
      return [];
    }
  }
  ...
  Future<void> fetchIntakeData() async {
    try {
      QuerySnapshot querySnapshot =
          await FirebaseFirestore.instance.collection('dropdown_data').get();
      List<String> items = [];
      print(items);
      querySnapshot.docs.forEach((doc) {
        final data = doc.data() as Map<String, dynamic>;
        final intakeData = data['intake'];
        if (intakeData != null && intakeData is List) {
          items.addAll(intakeData.map((e) => e.toString()));
        }
      });
      setState(() {
        intake_dropdownItems = items;
      });
    } catch (e) {
  ...
  Future<void> createAccount() async {
    ...
    if (!studentId.startsWith("SUKD")) {
      print("Invalid student ID format. It should start with 'SUKD'.");
      Navigator.pop(context); // Dismiss loading circle
      const Dialog_ok(
              title: 'Student ID format is incorrect',
              content: 'Please enter again your student id.')
          .show(context);
      return;
      // You can replace `print` with any other way to show the message, such as a dialog or a toast.
    }
...
''',
  ...
  // }
  bool isValidPhoneNumber(String phoneNumber) {
    // Regular expression for a phone number format (10 or 11 digits)
                      ...
                      child: Column(
                        children: [
                          ...
                          CreateAccountDropdown(
                            labelText: 'Programme',
                            hintText: 'Choose a programme',
                            controller: _programmeController,
                            dropdownItems:
                                programmeSpecializations.keys.toList(),
                            onChanged: (value) async {
                              setState(() {
                                if (value == null) {
                                  ...
                                  specialization_dropdownItems =
                                      programmeSpecializations[value] ?? [];
                                  // Clear the selected specialization if it's not in the new list
                                  if (!specialization_dropdownItems.contains(
                                      _specializationController.text)) {
                                    _specializationController.text = '';
                                  }
                                }
                          ...
                          CreateAccountDropdown(
                            ...
                            hintText: selectedProgramme.isEmpty
                          ...
                          CreateAccountDropdown(
                            ...
                            onChanged: (value) {
                              // Handle selected specialization if needed
                            },
                          ),
